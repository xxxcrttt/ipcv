...
double intersection_over_union(int X, int Y, int W, int H) {
	double final_IOU = 0;
	for (int i = 0; i < faces.size(); i++) {
		...
		int area_G = faces[i].width * faces[i].height;
		int area_R = W * H;

		int Union = area_G + area_R - intersection;

		double IOU = (double) intersection / (double) Union;
		//Comparing all red with green, take the largest IOU
...
int printIOU() {
	...
	for (int i = 0; i < n; i++) {
		double result = intersection_over_union(truex[i], truey[i], width[i], height[i]);
		if (result >= threshold) {
			correct++;
		}
	}
	std::cout << "Number of correctly detected faces : " << correct << std::endl;
	return correct;
}

//precision：correct green/all green   TP/TP+FP
//recall：correct green/all red      TP/TP+FN
double f1score(double precision, double recall) {
	double f1score = 2 * ((precision * recall) / (precision + recall));
	std::cout << "F1score of this image is "<< f1score << std::endl;
	return f1score;
}
